...
SanitizerMask parseArgValues(const Driver &D, const llvm::opt::Arg *A,
  ...
  for (int i = 0, n = A->getNumValues(); i != n; ++i) {
    ...
    // ClusterTag. Check for hwaddress usage and report error
    if (A->getOption().matches(options::OPT_fsanitize_EQ) &&
        0 == strcmp("hwaddress", Value) && DiagnoseErrors) {
      D.Diag(clang::diag::err_drv_hwaddress_disabled_for_clustertag);
      return SanitizerMask(); // Return empty mask to prevent further processing
    }
