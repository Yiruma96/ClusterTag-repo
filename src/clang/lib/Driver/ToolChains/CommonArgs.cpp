...
void tools::addArchSpecificRPath(const ToolChain &TC, const ArgList &Args,
  ...
  // ClusterTag. Also enable for sanitizers using shared libraries (e.g., HWASan, ClusterTag).
  const SanitizerArgs &SanArgs = TC.getSanitizerArgs(Args);
  const bool NeedsSanitizerRPath = SanArgs.needsSharedRt() &&
      (SanArgs.needsHwasanRt() || SanArgs.needsAsanRt() || SanArgs.needsTsanRt() ||
       SanArgs.needsMsanRt() || SanArgs.needsUbsanRt());
  bool DefaultValue = IsVE || NeedsSanitizerRPath;
...
static void addSanitizerRuntime(const ToolChain &TC, const ArgList &Args,
  ...
  if (IsShared) {
    // ClusterTag. For shared sanitizer runtimes, always add rpath to the directory containing the library
    SmallString<128> LibPath(TC.getCompilerRT(Args, Sanitizer, ToolChain::FT_Shared));
    llvm::sys::path::remove_filename(LibPath);
    if (TC.getVFS().exists(LibPath)) {
      CmdArgs.push_back("-rpath");
      CmdArgs.push_back(Args.MakeArgString(LibPath));
    }
    // Also try the general arch-specific path for compatibility
    addArchSpecificRPath(TC, Args, CmdArgs);
  }
}
