...
// ClusterTag. Used to initialize global variables in the tool class
#include "sanitizer_common/my_tools.h"
...
__attribute__((constructor(0))) void __hwasan_init() {
  ...
  // ClusterTag. Controls the density of Cluster randomization, defaults to 5.
  const char* pool_density = GetEnv("ct_pool_density");
  if (pool_density != nullptr){
    __sanitizer::ct_pool_density = stringToInteger(pool_density);
    Printf("[CT] Set ct_pool_density=%s\n", pool_density);
  } else
    __sanitizer::ct_pool_density = 5;

  InitInstrumentation();

  // ClusterTag. Set up global variables
  __sanitizer::ct_shadow_memory_dynamic_address = __hwasan_shadow_memory_dynamic_address;
  __sanitizer::ct_shadow_memory_dynamic_end = MemToShadow(kHighMemEnd);
  __sanitizer::ct_AddressTagMask = kAddressTagMask;
  __sanitizer::ct_AddressTagShift = kAddressTagShift;
  __sanitizer::ct_ShadowScale = kShadowScale;
  __sanitizer::ct_randomize_region = false;
  __sanitizer::my_random_state = genRandomSeed();
  __sanitizer::page_size_ = GetPageSizeCached();
  __sanitizer::free_list_interval_ns = 5 * 1000000000ULL;

  InitLoadedGlobals();

  // ClusterTag. Use static_assert to check at compile time that CTLargeChunkHeader is 16 bytes, otherwise report an error
  static_assert(sizeof(CTLargeChunkHeader) == 16, "CTChunkHeader must be 16 bytes");
  ...
  // ClusterTag. Initialize our own persistent allocator
  CTPersistentAllocatorInit(&ct_persistent_allocator);
