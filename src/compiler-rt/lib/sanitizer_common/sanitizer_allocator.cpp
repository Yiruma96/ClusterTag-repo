...
InternalAllocator *internal_allocator() {
  Printf("The program is using internal_allocator to attempt memory management, please debug to confirm the call site\n");
  internal__exit(1);
...
static void *RawInternalAlloc(uptr size, InternalAllocatorCache *cache,
  ...
  if (cache == 0) {
    ...
    return internal_allocator()->Allocate(&internal_allocator_cache, size, alignment);
  }
