...
class SizeClassAllocator32 {
 ...
 public:
  ...
  // ClusterTag. ================Keep the same function declarations as in Primary64================

  bool is_clustertag;

  void init_for_clustertag() {
    Printf("Cannot call init_for_clustertag function in SizeClassAllocator32\n");
    internal__exit(1);
  }

  NOINLINE bool CTRefill(CTPerClass *c, uptr class_id) {
    Printf("Cannot call CTRefill function in SizeClassAllocator32\n");
    internal__exit(1);
  }

  uptr HWASanAllocateNewRegion(uptr class_id) {
    Printf("Cannot call HWASanAllocateRegion function in SizeClassAllocator32\n");
    internal__exit(1);
  }

  void* HWASanAllocateLargeChunk(uptr map_size, uptr* res_beg) {
    Printf("Cannot call HWASanAllocateLargeChunk function in SizeClassAllocator32\n");
    internal__exit(1);
  }

  void HWASanDeallocateLargeChunk(void *p) {
    Printf("Cannot call HWASanDeallocateLargeChunk function in SizeClassAllocator32\n");
    internal__exit(1);
  }
