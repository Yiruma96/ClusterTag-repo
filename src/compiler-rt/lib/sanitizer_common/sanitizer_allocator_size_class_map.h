  ...
  static void Print() {
    ...
    for (uptr i = 0; i < kNumClasses; i++) {
      ...
      if (i == kBatchClassID)
        d = p = l = 0;
      Printf(
          "c%02zu => s: 0x%lx diff: +0x%lx %02zu%% l %zu cached: %zu %zu; id %zu\n",
...
// ClusterTag. template <uptr kNumBits, uptr kMinSizeLog, uptr kMidSizeLog, uptr kMaxSizeLog, uptr kMaxNumCachedHintT, uptr kMaxBytesCachedLog>
//   kMinSizeLog: Defines the initial size in the ClassMap starting from 2^kMinSizeLog.
//   kMidSizeLog: Between 2^kMinSizeLog and 2^kMidSizeLog, the size increases by 2^kMinSizeLog each step.
//   kMaxSizeLog: Between 2^kMidSizeLog and 2^kMaxSizeLog, the size increases by 2^(kMidSizeLog-kNumBits) each step. For example, if kMidSizeLog is 0b10000000 and kNumBits is set to 2, the increment is 100000 each step.
//   kNumBits: Defines the step size after reaching 2^kMidSizeLog, which is 2^(kMidSizeLog-kNumBits).
...
// ClusterTag. Define a custom ClassMap, where the allocation chain is aligned according to 2^k.
#define POWERMAXSIZELOG 16
typedef SizeClassMap<1, 3, 3, POWERMAXSIZELOG, 256, 14> Power2SizeClassMap;
