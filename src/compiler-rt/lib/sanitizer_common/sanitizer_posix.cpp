...
// ClusterTag. Used for stack printing
#include <execinfo.h>
...
void UnmapOrDie(void *addr, uptr size) {
  ...
  int rverrno = 0;
  if (UNLIKELY(internal_iserror(res, &rverrno))) {
    Report("ERROR: %s failed to deallocate 0x%zx (%zd) bytes at address %p-0x%lx 错误码res=%d\n",
           SanitizerToolName, size, size, addr, (uptr)addr+size, rverrno);
