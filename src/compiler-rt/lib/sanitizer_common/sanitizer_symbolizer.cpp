...
void AddressInfo::Clear() {
  ...
  CTPersistentAllocatorFree(&ct_persistent_allocator, (uptr)module, 1);
  CTPersistentAllocatorFree(&ct_persistent_allocator, (uptr)function, 1);
  CTPersistentAllocatorFree(&ct_persistent_allocator, (uptr)file, 1);
  internal_memset(this, 0, sizeof(AddressInfo));
  function_offset = kUnknown;
  uuid_size = 0;
}
...
SymbolizedStack *SymbolizedStack::New(uptr addr) {
  ...
  // ClusterTag. Use our persistent allocator
  void *mem = (void*)CTPersistentAllocatorAllocate(&ct_persistent_allocator, sizeof(SymbolizedStack));
...
void SymbolizedStack::ClearAll() {
  ...
  // InternalFree(this);
  // ClusterTag. Use our persistent allocator
  CTPersistentAllocatorFree(&ct_persistent_allocator, (uptr)this, 1);
}
...
void DataInfo::Clear() {
  ...
  CTPersistentAllocatorFree(&ct_persistent_allocator, (uptr)module, 1);
  CTPersistentAllocatorFree(&ct_persistent_allocator, (uptr)file, 1);
  CTPersistentAllocatorFree(&ct_persistent_allocator, (uptr)name, 1);
  internal_memset(this, 0, sizeof(DataInfo));
}
...
void FrameInfo::Clear() {
  ...
  for (LocalInfo &local : locals) {
    ...
    // ClusterTag. Use our persistent allocator
    CTPersistentAllocatorFree(&ct_persistent_allocator, (uptr)local.function_name, 1);
    CTPersistentAllocatorFree(&ct_persistent_allocator, (uptr)local.name, 1);
    CTPersistentAllocatorFree(&ct_persistent_allocator, (uptr)local.decl_file, 1);
  }
  locals.clear();
}
