...
const char *ExtractToken(const char *str, const char *delims, char **result) {
  ...
  *result = (char *)CTPersistentAllocatorAllocate(&ct_persistent_allocator, prefix_len + 1);
...
const char *ExtractInt(const char *str, const char *delims, int *result) {
  ...
  // InternalFree(buff);
  // ClusterTag. Use our persistent allocator
  CTPersistentAllocatorFree(&ct_persistent_allocator, (uptr)buff, 1);
  return ret;
}
...
static const char *ParseFileLineInfo(AddressInfo *info, const char *str) {
  ...
  // ClusterTag. Use our persistent allocator
  CTPersistentAllocatorFree(&ct_persistent_allocator, (uptr)file_line_info, 1);
  return str;
}
...
void ParseSymbolizePCOutput(const char *str, SymbolizedStack *res) {
  ...
  while (true) {
    ...
    if (function_name[0] == '\0') {
      ...
      // ClusterTag. Use our persistent allocator
      CTPersistentAllocatorFree(&ct_persistent_allocator, (uptr)function_name, 1);
      break;
    }
    SymbolizedStack *cur;
    if (top_frame) {
    ...
    if (0 == internal_strcmp(info->function, "??")) {
      ...
      // ClusterTag. Use our persistent allocator
      CTPersistentAllocatorFree(&ct_persistent_allocator, (uptr)info->function, 1);
      info->function = 0;
    }
    if (info->file && 0 == internal_strcmp(info->file, "??")) {
      // InternalFree(info->file);
      // ClusterTag. Use our persistent allocator
      CTPersistentAllocatorFree(&ct_persistent_allocator, (uptr)info->file, 1);
      info->file = 0;
    }
  }
}
